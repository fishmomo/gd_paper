...
def process_file(file_path, country_station_id, indice_valid, precip_ncpath):
    ...
    YYMMDDhhmmss = re.findall(r'\d{14}', filename)[0]
    YY = YYMMDDhhmmss[:4]
    MM = YYMMDDhhmmss[4:6]
    DD = YYMMDDhhmmss[6:8]
...
def main():
    ...
    file_list = sorted(list(data_dir1.glob("*5959_4000M_V0001.pkl.gz"))+list(data_dir2.glob("*5959_4000M_V0001.pkl.gz")))
    if not file_list:
    ...
    precip_all.to_csv(r"H:\YeWu\Zhou\guangzhou\warmcloud_allprecip.csv")
